...
/* detecta si es un dispositivo y si el dispositivo es iphone */
...
var phoneFormat = function () {
  ...
  _createClass(phoneFormat, [{
    ...
    value: function thisKeyUp() {
      ...
      jQuery(this.thisPhone).keyup(function () {
...
/*
var message = 'Error en la comunicación: Lo sentimos, se presentó un error tratando de realizar la compra. Por favor intenta de nuevo.';
mainAlert.positionalert = 'static';
mainAlert.alerttype = 'red';
mainAlert.msg = message;
mainAlert.time = true;
mainAlert.controls();
*/
...
/*
var modDudas = new modsDudas(jQuery('.wrrp_modsopc'));
modDudas.controls()
*/
...
var assembleSegment = function () {
  function assembleSegment() {
    ...
    this.event, this.userId, this.returnProperties = {};
  }
...
jQuery(document).ready(function () {
  ...
  jQuery("#container_shopweb").shopWeb({
    srcImages: globalSrcImages,
    base_path: globalBasePath,
    numPhone: globalNumPhone,
    balance: globalBalance,
    deuda: globalDeuda,
    creditoDisponible: globalCreditoDisponible,
    balanceEntero: globalBalanceEntero,
    alert_responce: globalAlert_responce,
    packageId: globalPackageId,
    packageTitle: globalPackageTitle,
    packageCost: globalPackageCost
  });
});

/* mensaje opt */

var otpMessage = function () {
  function otpMessage() {
    _classCallCheck(this, otpMessage);
  }

  _createClass(otpMessage, [{
    key: "optSend",
    value: function optSend(phoneNUmber, msotp) {
      jQuery.ajax({
        type: "POST",
        //url:  "http://10.67.223.149:8888/transaccionales-co/tigopaquete/otp_send",
        url: Drupal.url("tigopaquete/otp_send"),
        //url: 'http://localhost/transaccionales-co/tigopaquete/otp_send',
        data: { num_lin: phoneNUmber },
        success: function success(response) {

          // Segment
          var loadConfirmationCode = {
            category: 'Landing Paquetes',
            label: 'Solicitar nuevo'
          };
          segmentSend.userId = GetUserId();
          segmentSend.event = 'LCP - Código de confirmación';
          segmentSend.returnProperties = loadConfirmationCode;
          segmentSend.sendSegment();
          // Segment

          if (newCookieCode.cookieExist()) {
            // si ya existe un codigo activo
            clearInterval(newCookieCode.intervalCookie);
            newCookieCode.deleteCookie();
            setTimeout(function () {
              newCookieCode.createCookie();
              newCookieCode.checkCookieExist();
            }, 1000);
          } else {
            newCookieCode.createCookie();
            newCookieCode.checkCookieExist();
          ...
          switch (msotp) {
            case 'msotp1':
              ...
              break;
            case 'msotp2':
              modalRegistraNumero.msotp2(response);
              break;
            case 'msotp3':
              modalregistro.msotp3(response);
              break;
            case 'msotp4':
              modalregistro.msotp4(response);
              break;
          }
...
var newOtp = function () {
  function newOtp() {
    _classCallCheck(this, newOtp);
  }

  _createClass(newOtp, [{
    key: "optSend",
    value: function optSend(arrayOtp, msotpsend) {
      var responseotp;
      jQuery.ajax({
        type: "POST",
        url: Drupal.url("tigopaquete/otp"),
        //url:  "http://10.67.223.149:8888/transaccionales-co/tigopaquete/otp",
        //url: 'http://localhost/transaccionales-co/tigopaquete/otp',
        data: arrayOtp,
        success: function success(response) {
          ...
          switch (msotpsend) {
            case 'msotpsend1':
              modalRegistraNumero.msotpsend1(response);
              break;
            case 'msotpsend2':
              modalregistro.msotpsend2(response);
              break;
          }
        },
        error: function error(jqXHR, textStatus, errorThrown) {
...
var deleteSuscripcion = function () {
  function deleteSuscripcion() {
    _classCallCheck(this, deleteSuscripcion);
  }

  _createClass(deleteSuscripcion, [{
    key: "fnDelSus",
    value: function fnDelSus(idSus, thisSus) {
      jQuery.ajax({
        type: "POST",
        //url:  "http://10.67.223.149:8888/tigopaquete/+"+idSus+"/delete_subscription",
        url: Drupal.url("tigopaquete/" + idSus + "/delete_subscription"),
        success: function success(response) {
          if (response.status == 1) {
            jQuery(thisSus).parents('.li_susc').slideUp(function () {
              jQuery(this).remove();
            });
...
function msotp1(response) {
  // validacion 1

  // Segment
  var loadPhoneNumberData = {
    category: 'Landing Paquetes',
    label: 'Consultar Número'
  };
  segmentSend.userId = GetUserId();
  segmentSend.event = 'LCP - Consultar';
  segmentSend.returnProperties = loadPhoneNumberData;
  segmentSend.sendSegment();
  // Segment

  var responseTrin = response.status.toString().trim();
  //var siCaptcha = response.captcha.toString().trim(); 

  if (responseTrin != '1') {
    if (captcha1.cookieExist()) {
      captcha1.checkCookieExist();
    } else {
      captcha1.createCookie();
    }
  }
  ...
  switch (responseTrin) {
    case '0':
      ...
      mainAlert.alerttype = 'red';
      mainAlert.msg = message;
      mainAlert.time = false;
      mainAlert.controls();
      break;
    case '1':
      mainAlert.closemodal();
      modalRegistraNumero.numberPhone = jQuery("#num_lin").attr('data-number');
      modalRegistraNumero.numberPhoneEdit = jQuery("#num_lin").val();
      modalRegistraNumero.openModal();
      break;
    case '2':
      jQuery('#num_lin').parent('.wrrp_campo').addClass('errorcampo');
      jQuery('#num_lin').parent('.wrrp_campo').find('.alerta_errorcampo').html('El número de línea que ingresaste no es correcto. Veríficalo e intenta de nuevo.');
      break;
    default:
...
var activeTabs = function () {
  function activeTabs(padre, numPhone) {
    _classCallCheck(this, activeTabs);

    this.padre = padre, this.btTab = padre.find('a'), this.activeTab = padre.find('.active_tits'), this.wrppTabs = jQuery('.wrrp_tabs_shop'), this.slider, this.thisActive, this.ullista = jQuery('.ul_paquetes_shop'), this.liLista = jQuery('.ul_paquetes_shop li'), this.btAcordeon = jQuery('.ul_paquetes_shop li .header_paquete_shop'), this.contAcordeon = jQuery('.ul_paquetes_shop li .acor_shop'), this.sliderActive = false, this.numPhone = numPhone;
  }

  _createClass(activeTabs, [{
    key: "animaList",
    value: function animaList(tab) {
      var liActive = jQuery(tab).find('.ul_paquetes_shop li');
      var thisDelay = 0;
      for (var i = 0; i < liActive.length; i++) {
        thisDelay += 0.25;
        TweenLite.to(jQuery(liActive).eq(i), 0.5, { css: { opacity: 1 }, delay: thisDelay });
      }
    }
  }, {
    key: "createSlider",
    value: function createSlider(tab) {
      var controlsSlider = jQuery(tab).find('.ul_paquetes_shop li').length > 3;
      this.slider = jQuery(tab).find('.ul_paquetes_shop').bxSlider({
        slideWidth: 312,
        infiniteLoop: false,
        controls: false,
        minSlides: 1,
        maxSlides: 3,
        pager: controlsSlider,
        auto: false,
        autoHover: false,
        slideMargin: 10
      });
    ...
    key: "controlsTabs",
    value: function controlsTabs(bt) {
      var that = this;
      var btPosition = jQuery(bt).position();
      var btWidth = jQuery(bt).innerWidth() + 10;
      var activeTab = this.activeTab;
      TweenLite.to(activeTab, 0.5, { css: { left: btPosition.left - 5, width: btWidth }, ease: Power3.easeOut });
    }
  }, {
    key: "controls",
    value: function controls() {
      ...
      var that = this;
      var btTab = this.btTab;
      var btActivo = this.padre.find('#' + this.thisActive).addClass('active');
      var thisType = jQuery(btActivo).attr("id");
      var dinamicType = "#tab_" + thisType;

      jQuery(this.btTab).each(function () {
        var idBoton = jQuery(this).attr('id');
        var idTab = "tab_" + idBoton;
        if (!jQuery('#' + idTab + ' li').length) {
          jQuery('#' + idBoton).parent('li').remove();
        }
      });

      jQuery(btTab).on('click', function (e) {
        e.preventDefault();
        btActivo = jQuery(this);
        if (!jQuery(this).hasClass('active')) {
          that.wrppTabs.removeClass('active');
          that.liLista.removeAttr('style');
          jQuery(btTab).removeClass('active');
          jQuery(this).addClass('active');
          thisType = jQuery(btActivo).attr("id");
          dinamicType = "#tab_" + thisType;
          that.controlsTabs(jQuery(btActivo));
          jQuery(that.wrppTabs).css('display', 'none');

          // Segment
          var thisCat = jQuery(this).html();
          var tabSelected = {
            category: 'Landing Paquetes',
            label: thisCat
          };
          segmentSend.userId = GetUserId();
          segmentSend.event = 'LCP - Click visualizar Paquetes';
          segmentSend.returnProperties = tabSelected;
          segmentSend.sendSegment();
          // Segment

          if (that.sliderActive) {
            that.slider.destroySlider();
            that.liLista.css('opacity', '0');
            jQuery(dinamicType).fadeIn(function () {
              that.createSlider(jQuery(dinamicType));
              that.animaList(jQuery(dinamicType));
            });
          } else {
            that.liLista.css('opacity', '0');
            jQuery(dinamicType).fadeIn(function () {
      ...
      jQuery(this.btAcordeon).on('click', function (e) {
        e.preventDefault();
        var nextContent = jQuery(this).next('.acor_shop');
        if (!that.sliderActive) {
          if (!jQuery(this).hasClass('active')) {
            jQuery(that.btAcordeon).removeClass('active');
            jQuery(this).addClass('active');
            jQuery(that.liLista).removeClass('active');
            jQuery(this).parents('li').addClass('active');
            jQuery(that.contAcordeon).slideUp();
            jQuery(nextContent).slideDown();
          } else {
            jQuery(that.btAcordeon).removeClass('active');
            jQuery(that.contAcordeon).slideUp();
          }
      ...
      setTimeout(function () {
        that.controlsTabs(jQuery(btActivo), dinamicType);
      }, 300);
      that.liLista.css('opacity', '0');
      jQuery(dinamicType).fadeIn(function () {
        if (window.innerWidth >= 960) {
          that.sliderActive = that.createSlider(jQuery(dinamicType));
        }
        that.animaList(jQuery(dinamicType));
      });
      jQuery(window).resize(function () {
        that.controlsTabs(jQuery(btActivo));
        that.liLista.css('opacity', '1');
        if (window.innerWidth >= 960 && that.sliderActive == false) {
          that.sliderActive = that.createSlider(jQuery(dinamicType));
          jQuery(that.btAcordeon).removeClass('active');
          jQuery(that.contAcordeon).removeAttr('style');
        } else if (window.innerWidth < 960 && that.sliderActive == true) {
          that.slider.destroySlider();
...
var fnCaptcha = function () {
  function fnCaptcha(idcaptcha, typeCaptcha) {
    _classCallCheck(this, fnCaptcha);

    this.idCaptcha = idcaptcha, this.typeCaptcha = typeCaptcha, this.arrIds = [], this.activeCaptcha = false, this.valCap = false, this.firstCaptcha = null, this.secondCaptcha = null, this.contador = 0;
  }

  _createClass(fnCaptcha, [{
    key: "createCaptcha",
    value: function createCaptcha() {
      if (!this.valCap) {
        this.valCap = true;
        this.firstCaptcha = grecaptcha.render(this.arrIds[0], {
          'sitekey': '6LewmDUUAAAAAE15BkiZKiIGKUDn1Ox9AmLORrqp'
        });
        this.secondCaptcha = grecaptcha.render(this.arrIds[1], {
          'sitekey': '6LewmDUUAAAAAE15BkiZKiIGKUDn1Ox9AmLORrqp'
        });
      };
      this.activeCaptcha = true;
      if (this.typeCaptcha == 'phone') {
        jQuery('.header_shopweb').addClass('header_captcha');
        jQuery('#reg_numcaptcha2').css('display', 'block');
      }
      if (this.typeCaptcha == 'otp') {
        jQuery('#reg_otpcaptcha1').css('display', 'block');
        jQuery('#reg_otpcaptcha2').css('display', 'block');
      }
    }
  }, {
    key: "resetCaptcha",
    value: function resetCaptcha() {
      ...
      this.deleteCookie();
      jQuery('.header_shopweb').removeClass('header_captcha');
      jQuery('#reg_numcaptcha2').css('display', 'none');
      jQuery('#reg_otpcaptcha1').css('display', 'none');
      jQuery('#reg_otpcaptcha2').css('display', 'none');
    }
  }, {
    key: "destroyCaptcha",
    value: function destroyCaptcha() {
      this.activeCaptcha = false;
      this.deleteCookie();
    }
  }, {
    key: "validateCaptcha",
    value: function validateCaptcha(responseCaptcha) {
      var resCap;
      if (responseCaptcha.length == 0) resCap = false;else resCap = true;
      return resCap;
    }
  }, {
    key: "deleteCookie",
    value: function deleteCookie() {
      ...
      document.cookie = this.idCaptcha + '=; expires=' + now.toUTCString() + '; path=/;';
    }
  }, {
    key: "cookieExist",
    value: function cookieExist() {
      var thereIsCookie = false;
      var theCookies = document.cookie;
      if (theCookies.indexOf(this.idCaptcha) != -1) {
        thereIsCookie = true;
      } else {
        thereIsCookie = false;
      }
    ...
    value: function createCookie() {
      ...
      document.cookie = this.idCaptcha + '=1; expires=' + now.toGMTString() + ';path=/';
    }
  }, {
    ...
    value: function checkCookieExist() {
      ...
      //var cookies_data = currentCookies[1].split(";");
      var now = new Date();
      var time = now.getTime();
      var expireTime = time + 3600 * 1000 * 24;
      now.setTime(expireTime);
      ...
      for (var i in lista) {
        var busca = lista[i].search(that.idCaptcha);
        if (busca > -1) {
          var micookie = lista[i];
          var params = {};
          var igual = micookie.split("=");
          params[igual[0]] = igual[1];
          var valor = igual[1];
          var newValue = parseInt(igual[1]) + 1;
          that.contador = newValue;
          document.cookie = that.idCaptcha + "=" + newValue + '; expires=' + now.toGMTString() + ';path=/';
          if (newValue >= 3 && !that.activeCaptcha) {
            that.createCaptcha();
          }
        }
      }
...
var modalRegisterNumber = function () {
  function modalRegisterNumber() {
    _classCallCheck(this, modalRegisterNumber);

    this.numberPhone = '', this.numberPhoneEdit = '', this.modalActive = jQuery('#mod_regnumber'), this.valActivation = false, this.messageReSend = 'Te volvimos a enviar el código de verificación. Ingrésalo.', this.alertaModal = this.modalActive.find('.wrapper_alerta'), this.btCloseModal = this.modalActive.find('.close_modal'), this.btresend = this.modalActive.find('#btResend'), this.ingresaPhone2, this.tl = new TimelineMax({ repeat: 2 });
  }

  _createClass(modalRegisterNumber, [{
    key: "msotp2",
    value: function msotp2(response) {
      ...
      switch (responseTrin) {
        case '0':
          jQuery(this.alertaModal).removeClass('blue_alert');
          jQuery(this.alertaModal).addClass('red_alert').css('display', 'block');
          jQuery(this.alertaModal).find('p').html(this.messageError());
          that.muevealerta();
          break;
        case '1':
          jQuery(this.alertaModal).removeClass('red_alert');
          jQuery(this.alertaModal).addClass('blue_alert').css('display', 'block');
          jQuery(this.alertaModal).find('p').html(this.messageReSend);
          that.muevealerta();
          break;
        case '2':
          jQuery(this.alertaModal).removeClass('blue_alert');
    ...
    key: "msotpsend1",
    value: function msotpsend1(response) {

      if (response.status != 1) {
        if (captcha2.cookieExist()) {
          captcha2.checkCookieExist();
        } else {
          captcha2.createCookie();
        }
      }
      if (response.status == 1) {
        jQuery('.container_loadbig').fadeIn(function () {
          setTimeout(function () {
            location.reload();
          }, 500);
        });
      } else {
        jQuery("#cod_act").parent('.wrrp_campo').addClass('errorcampo');
        jQuery("#cod_act").parent('.wrrp_campo').find('.alerta_errorcampo').html(this.messageError('codigo'));
      }
    }
  }, {
    key: "muevealerta",
    value: function muevealerta() {
      this.tl.to(this.alertaModal, 0.05, { rotation: 1 }).to(this.alertaModal, 0.05, { rotation: 0 }).to(this.alertaModal, 0.05, { rotation: -1 }).to(this.alertaModal, 0.05, { rotation: 0 });
    }
  }, {
    key: "messageSuccess",
    value: function messageSuccess(phone) {
      this.alertaModal.removeClass('red_alert');
      this.alertaModal.addClass('blue_alert');
      var message = 'Hemos enviado a la línea ' + phone + ' un mensaje de texto con el código de verificación.';
      return message;
    }
  }, {
    key: "messageError",
    value: function messageError(issue) {
      var message = '';
      switch (issue) {
        case 'codigo':
          message = 'El código que has ingresado no es válido.';
          break;
        case 'novalido':
          message = 'El código que has ingresado no es válido';
          break;
        case 'numero':
          message = 'El número no es correcto, intenta de nuevo';
          break;
        case 'captcha':
          message = 'Dale un click en "No soy un Robot" para continuar con la transacción..';
          break;
        default:
    ...
    key: "valVerificar",
    value: function valVerificar() {
      if (this.valActivation) {
        jQuery('#bt-contver').removeClass('inactive');
        this.alertaModal.find('.wrrp_campo').removeClass('novalido');
      } else {
        jQuery('#bt-contver').addClass('inactive');
      }
    }
  }, {
    key: "openModal",
    value: function openModal() {
      ...
      if (esmobile.itsMobile) {
        jQuery('html').addClass('body_hidden');
        jQuery('body').addClass('body_hidden');
      }

      var that = this;
      this.alertaModal.css('display', 'block').find('p').html(this.messageSuccess(this.numberPhoneEdit));
      // clicks

      jQuery('.cover_container_modales').fadeIn(function () {
        jQuery('.container_modales').css('display', 'block');
        TweenLite.to(that.modalActive, 0.8, { css: { display: 'inline-block', top: 0, opacity: 1 }, ease: Power3.easeOut });
      });
      jQuery("#cod_act").keyup(function (e) {
        solonumeros.onlyNumbers(jQuery(this).val(), jQuery(this));
      });
      jQuery("#cod_act").keyup(function () {
        var valActual = jQuery(this).val();
        if (valActual.length === 4) {
          that.valActivation = true;
        } else {
          that.valActivation = false;
        }
        that.valVerificar();
      });

      jQuery("#cod_act").on('blur', function () {
        var valActual = jQuery(this).val();
        if (valActual.length === 4) {
          jQuery(this).parent('.wrrp_campo').removeClass('errorcampo');
        } else {
          jQuery(this).parent('.wrrp_campo').addClass('errorcampo');
          jQuery(this).parent('.wrrp_campo').find('.alerta_errorcampo').html(that.messageError('codigo'));
        }
      });

      this.btCloseModal.on('click', function (e) {
        e.preventDefault();

        if (esmobile.itsMobile) {
          jQuery('html').removeClass('body_hidden');
          jQuery('body').removeClass('body_hidden');
        }

        TweenLite.to(that.modalActive, 0.5, { css: { top: 50, opacity: 0, display: 'none' }, onComplete: function onComplete() {
            jQuery('.cover_container_modales').fadeOut(function () {
              jQuery("#cod_act").removeClass('errorcampo');
              jQuery('.container_modales').css('display', 'none');
              jQuery('#bt-contver').addClass('inactive');
              captcha1.resetCaptcha();
              captcha2.resetCaptcha();
              that.modalActive.find('.wrrp_campo').removeClass('errorcampo active');
              that.modalActive.find('input').each(function () {
                jQuery(this).val('');
              });
            });
          } });
      });
      this.btresend.on('click', function (e) {
        e.preventDefault();
        jQuery(that.alertaModal).css('display', 'block');
        if (ingresaPhone1.valPhone) {
          sendMessageOtp.optSend(that.numberPhone, 'msotp2');
        } else {
          jQuery("#cod_act").parent('.wrrp_campo').addClass('errorcampo');
          jQuery("#cod_act").parent('.wrrp_campo').find('.alerta_errorcampo').html(that.messageError('codigo'));
        }
      });
    }
  }, {
    key: "submitForm",
    value: function submitForm(thisButton) {
      var that = this;
      var thisButton = thisButton;
      if (!jQuery(thisButton).hasClass('inactive')) {
        var infoCode = {
          cod_act: jQuery("#cod_act").val(),
          idPackage: ''
        };
        if (captcha2.activeCaptcha) {
          if (captcha2.validateCaptcha(grecaptcha.getResponse(captcha2.firstCaptcha))) {
            justOtp.optSend(infoCode, 'msotpsend1');
          } else {
            jQuery("#cod_act").parent('.wrrp_campo').addClass('errorcampo');
            jQuery("#cod_act").parent('.wrrp_campo').find('.alerta_errorcampo').html(that.messageError('captcha'));
          }
        } else {
          justOtp.optSend(infoCode, 'msotpsend1');
        }
...
var modRegNumNoBal = function () {
  function modRegNumNoBal() {
    _classCallCheck(this, modRegNumNoBal);

    this.modalActive = '#mod_reg_sinbal', this.btSteps = '#bt-regsinbal', this.startSteps = true, this.thisHref = '', this.title = '', this.idPackage = '', this.price = '', this.priceOffer = '', this.numberPhone = '', this.numberPhoneEdit = '', this.hrefRespuesta = '', this.typePackage = '', this.valPhone = false, this.valActivation = false, this.stepBalence = jQuery(this.modalActive).find('.step_nobalance'), this.alertaModal = jQuery(this.modalActive).find('.wrapper_alerta'), this.messageReSend = 'Te volvimos a enviar el código de verificación. Ingrésalo', this.tl = new TimelineMax({ repeat: 2 });
  }

  _createClass(modRegNumNoBal, [{
    key: "msotp3",
    value: function msotp3(response) {
      ...
      switch (responseTrin) {
        case '0':
          jQuery(this.alertaModal).removeClass('blue_alert');
          jQuery(this.alertaModal).addClass('red_alert');
          jQuery(this.alertaModal).find('p').html(this.messageError('comunicacion'));
          jQuery(this.alertaModal).slideDown();
          this.muevealerta();
          break;
        case '1':
          this.sendPhone(true);
          jQuery(this.stepBalence).slideDown();
          this.startSteps = false;
          jQuery(this.btSteps).addClass('inactive');
          jQuery('#wrrp_step1').slideUp();
          //jQuery(this.btSteps).find('span').html('Comprar');
          break;
        case '2':
          jQuery('#numlinsin').parent('.wrrp_campo').addClass('errorcampo');
          jQuery('#numlinsin').parent('.wrrp_campo').find('.alerta_errorcampo').html(this.messageError('numero'));
          break;
        default:
          console.log('default');
      }
    ...
    key: "sendPhone",
    value: function sendPhone(succes) {
      var that = this;
      if (succes) {
        jQuery(that.alertaModal).removeClass('red_alert');
        jQuery(that.alertaModal).addClass('blue_alert');
        jQuery(that.alertaModal).find('p').html(that.messageSuccess(jQuery('#numlinsin').val()));
      } else {
        jQuery(that.alertaModal).removeClass('blue_alert');
        jQuery(that.alertaModal).addClass('red_alert');
        jQuery(that.alertaModal).find('p').html(that.messageError('codigo'));
      }
      jQuery(that.alertaModal).slideDown();
      that.muevealerta();
    }
  }, {
    key: "msotp4",
    value: function msotp4(response) {
      var that = this;
      var responseTrin = response.status.toString().trim();
      ...
      switch (responseTrin) {
        case '0':
          jQuery('#numlinsin').parent('.wrrp_campo').addClass('errorcampo');
          jQuery('#numlinsin').parent('.wrrp_campo').find('.alerta_errorcampo').html(this.messageError('codigo'));
          break;
        case '1':
          jQuery(this.alertaModal).removeClass('red_alert');
          jQuery(this.alertaModal).addClass('blue_alert').css('display', 'block');
    ...
    key: "successlfullPurchese",
    value: function successlfullPurchese() {
      var that = this;
      TweenLite.to(that.modalActive, 0.5, { css: { top: 50, opacity: 0, display: 'none' }, onComplete: function onComplete() {
          jQuery('.container_modales').fadeOut(function () {
            jQuery('#wrrp_step1').removeAttr('style');
            jQuery(that.modalActive).find('.wrrp_campo').removeClass('errorcampo active');
            jQuery(that.modalActive).find('input').each(function () {
              jQuery(this).val('');
            });
            modalconfirmation.title = that.title;
            modalconfirmation.thisHref = that.thisHref;
            modalconfirmation.priceOffer = that.priceOffer;
            modalconfirmation.price = that.price;
            modalconfirmation.typePackage = that.typePackage;
            modalconfirmation.localPhone = jQuery('#numlinsin').attr('data-number');
    ...
    value: function callSaldoModal() {
      ...
      TweenLite.to(that.modalActive, 0.5, { css: { top: 50, opacity: 0, display: 'none' }, onComplete: function onComplete() {
          jQuery('.container_modales').fadeOut(function () {
            ...
            modalnosaldo.tipo('saldootp');
            modalnosaldo.thisHref = that.thisHref;
            modalnosaldo.title = that.title;
            modalnosaldo.price = that.price;
            modalnosaldo.priceOffer = that.priceOffer;
            modalnosaldo.openModal();
          });
        } });
    }
  }, {
    key: "callNoAplicaModal",
    value: function callNoAplicaModal() {
      var that = this;
      TweenLite.to(that.modalActive, 0.5, { css: { top: 50, opacity: 0, display: 'none' }, onComplete: function onComplete() {
          jQuery('.container_modales').fadeOut(function () {
            jQuery(that.modalActive).find('.wrrp_campo').removeClass('errorcampo active');
    ...
    key: "msotpsend2",
    value: function msotpsend2(responseotp) {
      ...
      if (response.status == 1) {
        // bien el codigo
        if (response.estadoPaquete == 1) {
          // paquete disponible oferta
          if (parseInt(response.saldo) < parseInt(that.price)) {
            // si el no saldo alcanza 
            that.callSaldoModal();
          } else {
            // si el saldo alcanza 
            that.successlfullPurchese();
          }
        } else {
          // paquete no disponible oferta
          that.callNoAplicaModal();
          // modal no aplica
        }
      } else {
        // error en el codigo
        jQuery('#codactsin').parent('.wrrp_campo').addClass('errorcampo');
        jQuery('#codactsin').parent('.wrrp_campo').find('.alerta_errorcampo').html(this.messageError('codigo'));
      }
    }
  }, {
    key: "muevealerta",
    ...
    value: function messageError(issue) {
      ...
      switch (issue) {
        case 'codigo':
          message = 'El código que has ingresado no es válido, por favor genera uno nuevo para continuar con la compra';
          break;
        case 'numero':
          message = 'El número de línea que ingresaste no es correcto. Veríficalo e intenta de nuevo.';
          break;
        case 'comunicacion':
          message = 'Error en la comunicación: Lo sentimos, se presentó un error tratando de realizar la compra. Por favor intenta de nuevo.';
          break;
        case 'captcha':
          message = 'Dale un click en "No soy un Robot" para continuar con la transacción.';
          break;
    ...
    key: "valVerificarStep1",
    value: function valVerificarStep1() {
      var that = this;
      if (this.valPhone) {
        jQuery(that.btSteps).removeClass('inactive');
      } else {
        jQuery(that.btSteps).addClass('inactive');
      }
    }
  }, {
    key: "valVerificarStep2",
    value: function valVerificarStep2() {
      var that = this;
      if (this.valPhone && this.valActivation) {
        jQuery(that.btSteps).removeClass('inactive');
      } else {
        jQuery(that.btSteps).addClass('inactive');
      }
    ...
    value: function openModal() {
      var that = this;
      var modalActive = this.modalActive;
      var btSend = jQuery(modalActive).find('.btsend');
      var btReSend = jQuery(modalActive).find('#btResWithoutBal');
      var wrppTitle = jQuery(modalActive).find('h2').html(this.title);
      var wrppPrice = jQuery(modalActive).find('p.price').html('Valor $ ' + this.priceOffer);
      var btclosemodal = jQuery(modalActive).find('.close_modal');
      if (esmobile.itsMobile) {
        jQuery('html').addClass('body_hidden');
        jQuery('body').addClass('body_hidden');
      }
      if (this.startSteps) {
        jQuery(that.alertaModal).css('display', 'none');
        jQuery(that.stepBalence).css('display', 'none');
      }
      // clicks
      jQuery('.cover_container_modales').fadeIn(function () {
        jQuery('.container_modales').css('display', 'block');
        TweenLite.to(jQuery(modalActive), 0.8, { css: { display: 'inline-block', top: 0, opacity: 1 }, ease: Power3.easeOut });
      });

      function valSecNum(thatinput) {
        if (that.startSteps) {
          that.valVerificarStep1();
        } else {
          that.valVerificarStep2();
        }
      }
      jQuery('#numlinsin').keyup(function () {
        if (ingresaPhone1Modal.valPhone) {
          that.valPhone = true;
        } else {
          that.valPhone = false;
        }
      ...
      jQuery("#codactsin").keyup(function (e) {
        solonumeros.onlyNumbers(jQuery(this).val(), jQuery(this));
        var valActual = jQuery(this).val();
        if (valActual.length == 4) {
          that.valActivation = true;
      ...
      jQuery("#codactsin").on('blur', function () {
        var valActual = jQuery(this).val();
        if (valActual.length === 4) {
          jQuery(this).parent('.wrrp_campo').removeClass('errorcampo');
        } else {
      ...
      jQuery(btReSend).on('click', function (e) {
        e.preventDefault();
        if (ingresaPhone1Modal.valPhone) {
          sendMessageOtp.optSend(jQuery("#numlinsin").attr('data-number'), 'msotp4');
        } else {
          jQuery(that.alertaModal).removeClass('blue_alert');
          jQuery(that.alertaModal).addClass('red_alert');
          jQuery(that.alertaModal).find('p').html('El número ingresado es incorrecto');
        }
        that.muevealerta();
      });
    }
    ...
    key: "closeModal",
    value: function closeModal(boton) {
      var that = this;
      var btCerrar = boton;
      var eventSEg;
      if (esmobile.itsMobile) {
      ...
      if (jQuery(btCerrar).hasClass('buttonform_white')) {
        eventSEg = 'LCP - Cancelar';
      } else {
        eventSEg = 'LCP - Cerrar';
      }
      // Segment
      var continueCancelCloseBuyPacket = {
        category: 'Landing Paquetes',
        label: that.typePackage + " - " + that.idPackage + " - " + that.title + " - $" + that.priceOffer,
        value: that.price,
        currency: 'COL'
      };
      segmentSend.userId = GetUserId();
      segmentSend.event = eventSEg;
      segmentSend.returnProperties = continueCancelCloseBuyPacket;
      segmentSend.sendSegment();
      // Segment
      TweenLite.to(that.modalActive, 0.5, { css: { top: 50, opacity: 0, display: 'none' }, onComplete: function onComplete() {
          jQuery('.cover_container_modales').fadeOut(function () {
    ...
    key: "stepsModal",
    value: function stepsModal(btsteps) {
      var that = this;
      var btsteps = btsteps;
      if (!jQuery(btsteps).hasClass('inactive')) {
        if (that.startSteps) {
          //console.log('step1');
          ...
          segmentSend.userId = GetUserId();
          segmentSend.event = 'LCP - Continuar';
          segmentSend.returnProperties = continueCancelCloseBuyPacket;
          segmentSend.sendSegment();
          // Segment

          // captcha aqui
          if (captcha1.activeCaptcha) {
            if (captcha1.validateCaptcha(grecaptcha.getResponse(captcha1.secondCaptcha))) {
              sendMessageOtp.optSend(jQuery("#numlinsin").attr('data-number'), 'msotp3');
              grecaptcha.reset(captcha1.firstCaptcha);
              grecaptcha.reset(captcha1.secondCaptcha);
              captcha1.resetCaptcha();
            } else {
              jQuery('#numlinsin').parent('.wrrp_campo').addClass('errorcampo');
              jQuery('#numlinsin').parent('.wrrp_campo').find('.alerta_errorcampo').html('Dale un click en "No soy un Robot" para continuar con la transacción.');
            }
          } else {
            sendMessageOtp.optSend(jQuery("#numlinsin").attr('data-number'), 'msotp3');
          }
          ...
          //console.log('step2');
          var infoCode = {
            cod_act: jQuery("#codactsin").val(),
            idPackage: that.idPackage
          };
          if (captcha2.activeCaptcha) {
            if (captcha2.validateCaptcha(grecaptcha.getResponse(captcha2.secondCaptcha))) {
              justOtp.optSend(infoCode, 'msotpsend2');
            } else {

              jQuery('#codactsin').parent('.wrrp_campo').addClass('errorcampo');
              jQuery('#codactsin').parent('.wrrp_campo').find('.alerta_errorcampo').html('Dale un click en "No soy un Robot" para continuar con la transacción.');
            }
          } else {
            justOtp.optSend(infoCode, 'msotpsend2');
          }
  ...
  return modRegNumNoBal;
}();

/*Modal expiro*/


var modalExpiro = function () {
  function modalExpiro() {
    _classCallCheck(this, modalExpiro);

    this.modalActive = '#mod_expiro';
  }

  _createClass(modalExpiro, [{
    key: "openModal",
    value: function openModal() {
      var that = this;
      jQuery('.wrapper_modal').removeAttr('style');
      jQuery('.cont_modal_susc').removeAttr('style');
      var modalActive = this.modalActive;
      var alertaModal = jQuery(modalActive).find('.wrapper_alerta');
      jQuery(alertaModal).find('p').html('TU SESIÓN EXPIRÓ');

      var btClosedModal = jQuery('#mod_expiro').find('.close_modal');
      // clicks
      jQuery('.cover_container_modales').fadeIn(function () {
        jQuery('.container_modales').css('display', 'block');
        TweenLite.to(jQuery(modalActive), 0.8, { css: { display: 'inline-block', top: 0, opacity: 1 }, ease: Power3.easeOut });
      });
      jQuery(btClosedModal).on('click', function (e) {
        e.preventDefault();
        that.closeModal();
      });
    }
    ...
    value: function closeModal() {
      //location.reload();
      window.location.replace(Drupal.url('tigopaquete/destroy'));
    }
  }]);
...
var modalSinSaldo = function () {
  function modalSinSaldo() {
    _classCallCheck(this, modalSinSaldo);

    this.modalActive = '#mod_saldo', this.title = '', this.price = '', this.priceOffer = '', this.typeModal = '', this.modalSaldo = '', this.thisHref = '', this.alertaModal = jQuery('#mod_saldo').find('.wrapper_alerta'), this.copyAlert = '';
  }

  _createClass(modalSinSaldo, [{
    key: "tipo",
    value: function tipo(_tipo) {
      var that = this;
      var alertaModal = jQuery('#mod_saldo').find('.wrapper_alerta');
      var btRechageModal = jQuery('#mod_saldo').find('#bt-sinsaldomodal');
      switch (_tipo) {
        case 'saldo':
          jQuery(this.modalActive).find('h2').html('¡Tu saldo no es suficiente!');
          jQuery(this.modalActive).find('.copysinsaldo').html('Te invitamos a comprar este paquete recargando  en línea o comprando otro paquete de menor valor.');
          jQuery(btRechageModal).find('span').html('IR A RECARGAR EN LÍNEA');
          that.copyAlert = '¿No tienes saldo? Recarga ahora mismo y disfruta del paquete que más te guste', that.typeModal = 'saldo';
          jQuery(alertaModal).removeClass('red_alert');
          jQuery(alertaModal).addClass('blue_alert');
          break;
        case 'saldootp':
          jQuery(this.modalActive).find('h2').html('¡Tu saldo no es suficiente!');
          jQuery(this.modalActive).find('.copysinsaldo').html('Te invitamos a comprar este paquete recargando  en línea o comprando otro paquete de menor valor.');
          that.copyAlert = '¿No tienes saldo? Recarga ahora mismo y disfruta del paquete que más te guste';
          jQuery(btRechageModal).find('span').html('IR A RECARGAR EN LÍNEA');
          jQuery(alertaModal).removeClass('red_alert');
          jQuery(alertaModal).addClass('blue_alert');
          that.typeModal = 'saldootp';
          break;
        case 'noaplica':
          jQuery(this.modalActive).find('h2').html('¡No aplica para tu plan!');
          jQuery(this.modalActive).find('.copysinsaldo').html('El paquete que intentas comprar no aplica para tu plan, te invitamos a ver tus ofertas.');
          jQuery(btRechageModal).find('span').html('VER MIS OFERTAS');
          that.copyAlert = 'El paquete que intentas comprar no aplica para tu plan, te invitamos a ver tus ofertas.';
          jQuery(alertaModal).removeClass('blue_alert');
          jQuery(alertaModal).addClass('red_alert');
          that.typeModal = 'noaplica';
          break;
      }
    }
  }, {
    ...
    value: function openModal() {
      var that = this;
      var modalActive = this.modalActive;
      var numberPhoneEdit = this.numberPhoneEdit;
      var alertaModal = jQuery(modalActive).find('.wrapper_alerta');
      var wrppTitle = jQuery(modalActive).find('h2');
      var wrppPrice = jQuery(modalActive).find('p.price').html('Valor $ ' + this.priceOffer);
      jQuery(alertaModal).find('p').html(this.copyAlert);
      var btClosedModal = jQuery('#mod_saldo').find('.close_modal');
      var btRechageModal = jQuery('#mod_saldo').find('#bt-sinsaldomodal');
      // clicks
      jQuery('.cover_container_modales').fadeIn(function () {
        jQuery('.container_modales').css('display', 'block');
      ...
      jQuery(btRechageModal).on('click', function (e) {
        e.preventDefault();

        // Segment
        var goToRechargeLanding = {
          category: 'Landing Paquetes',
          label: 'Ir a landing recargas'
        };
        segmentSend.userId = GetUserId();
        segmentSend.event = 'LCP - Ir a Recargar';
        segmentSend.returnProperties = goToRechargeLanding;
        segmentSend.sendSegment();
        // Segment

        var typeModal = that.typeModal;
        var phoneNumber = globalNumPhone;

        var linkRecarga = 'http://www.tigo.com.co/servicios/recarga?client_id=042884dcdee8d77469c9d32fd5c53e6a&msisdn=' + phoneNumber;

        switch (that.typeModal) {
          case 'saldo':
            window.location.replace(linkRecarga);
            break;
          case 'saldootp':
            window.location.replace(linkRecarga);
            break;
      ...
      jQuery(btClosedModal).on('click', function (e) {
        ...
        switch (typeModal) {
          case 'saldo':
            TweenLite.to(jQuery(modalActive), 0.5, { css: { top: 50, opacity: 0, display: 'none' }, onComplete: function onComplete() {
                jQuery('.container_modales').css('display', 'none');
                jQuery('.cover_container_modales').fadeOut();
              } });
            break;
          case 'saldootp':
            jQuery('.container_loadbig').fadeIn(function () {
...
var modalConfirmation = function () {
  function modalConfirmation() {
    ...
    this.modalActive = '#mod_confirmation', this.btClosedModal = jQuery('#mod_confirmation').find('.close_modal');
    this.copySeguro = jQuery('#mod_confirmation').find('h2'), this.typePackage, this.catPaquete, this.thisHref, this.title, this.price, this.idPackage, this.priceOffer, this.localPhone;
  }

  _createClass(modalConfirmation, [{
    key: "numPhone",
    value: function numPhone() {
      var numLinea;
      if (globalNumPhone != "") {
        numLinea = iniShowPhone.formatoInput(globalNumPhone);
      } else {
        numLinea = iniShowPhone.formatoInput(this.localPhone);
      }
      return numLinea;
    }
  }, {
    key: "titleParameters",
    value: function titleParameters() {
      var that = this;
      var titleConf = '';
      switch (this.typePackage) {
        case 'PACKAGE':
          titleConf = "\xBFEst\xE1s seguro de comprar el Paquete " + that.title + " por valor de $" + that.priceOffer + " a la l\xEDnea " + that.numPhone();
          break;
        case 'SUBSCRIPTION':
          titleConf = "\xBFEst\xE1s seguro que deseas suscribirte al Paquete " + that.title + "\n        <br>\n        Cada vez que recargues se realizar\xE1 la compra autom\xE1tica de este paquete por valor de $" + that.priceOffer + " a la l\xEDnea " + that.numPhone() + "?";
          break;
        case 'ADELANTA TU PAQUETE':
          titleConf = "\xBFEst\xE1s seguro que quieres hacer el pr\xE9stamo del Paquete " + that.title + "\n        <br>\n        por valor de $" + that.priceOffer + " a la l\xEDnea " + that.numPhone() + "?";
          break;
        default:
          titleConf = "\xBFEst\xE1s seguro de comprar el Paquete " + that.title + " por valor de $" + that.priceOffer + " a la l\xEDnea " + that.numPhone();
      }
    ...
    value: function openModal() {
      var that = this;
      var btClosedModal = jQuery('#mod_confirmation').find('.close_modal');

      // clicks

      that.copySeguro.html(that.titleParameters());

      jQuery('.cover_container_modales').fadeIn(function () {
        jQuery('.container_modales').css('display', 'block');
        TweenLite.to(jQuery(that.modalActive), 0.8, { css: { display: 'inline-block', top: 0, opacity: 1 }, ease: Power3.easeOut });
      });

      /* Comprar */

      jQuery('#bt-buyconfirmation').on('click', function (e) {
        e.preventDefault();
        //Segment
        var buyProductValidate = {
          category: 'Validar Compra',
          label: that.typePackage + " - " + that.idPackage + " - " + that.title + " - " + that.price,
          value: that.price,
          currency: 'COL'
        };
        segmentSend.userId = GetUserId();
        segmentSend.event = 'LCP - Comprar - ' + that.catPaquete;
        segmentSend.returnProperties = buyProductValidate;
        segmentSend.sendSegment();
        //Segment
        window.location.replace(that.thisHref);
      });
      /* Comprar */
    }
  }, {
    ...
    value: function closeModal() {
      ...
      segmentSend.userId = GetUserId();
      segmentSend.event = 'LCP - Cancelar';
      segmentSend.returnProperties = continueCancelCloseBuyPacket;
      segmentSend.sendSegment();
      // Segment

      TweenLite.to(jQuery(that.modalActive), 0.5, { css: { top: 50, opacity: 0, display: 'none' }, onComplete: function onComplete() {
          jQuery('.container_modales').css('display', 'none');
          jQuery('.cover_container_modales').fadeOut();
        } });
    }
...
var modalSuscripciones = function () {
  function modalSuscripciones() {
    _classCallCheck(this, modalSuscripciones);

    this.modalActive = jQuery('#mod_subscripciones'), this.btClosedModal = jQuery('#mod_subscripciones').find('.close_modal');
  }

  _createClass(modalSuscripciones, [{
    key: "openModal",
    value: function openModal() {
      var that = this;
      // Segment
      var goToSubscriptions = {
        category: 'Landing Paquetes',
        label: 'Ver suscripciones'
      };
      segmentSend.userId = GetUserId();
      segmentSend.event = 'LCP - Ir a suscripciones';
      segmentSend.returnProperties = goToSubscriptions;
      segmentSend.sendSegment();
      // Segment
      ...
      // clicks
      jQuery('.cover_container_modales').fadeIn(function () {
        jQuery('.cont_modal_susc').css('display', 'block');
        TweenLite.to(that.modalActive, 0.8, { css: { display: 'inline-block', top: 0, opacity: 1 }, ease: Power3.easeOut });
      });

      jQuery(that.btClosedModal).on('click', function (e) {
        e.preventDefault();
        that.closeModal();
      });
    }
  }, {
    key: "acordeonSuscripcion",
    value: function acordeonSuscripcion(boton) {
      var thisBoton = boton;
      if (thisBoton.hasClass('active')) {
        jQuery('.header_mod_susc').removeClass('active');
        jQuery('.content_mod_susc').slideUp();
      } else {
        jQuery('.header_mod_susc').removeClass('active');
        thisBoton.addClass('active');
        jQuery('.content_mod_susc').slideUp();
        thisBoton.next('.content_mod_susc').slideDown();
      }
    }
  }, {
    key: "closeModal",
    value: function closeModal() {
      ...
      TweenLite.to(that.modalActive, 0.5, { css: { top: 50, opacity: 0, display: 'none' }, onComplete: function onComplete() {
          jQuery('.cover_container_modales').fadeOut(function () {
            jQuery('.cont_modal_susc').css('display', 'none');
          });
        } });
    }
  }]);

  return modalSuscripciones;
}();
/*Modal suscripciones*/
/*bt comprar*/

var fnBuyButton = function () {
  function fnBuyButton(parent) {
    _classCallCheck(this, fnBuyButton);

    this.parent = parent, this.buyBt = this.parent.find('.buybutton'), this.BuyActive = true;
  }

  _createClass(fnBuyButton, [{
    key: "phoneActivo",
    value: function phoneActivo() {
      var phoneActivo = false;
      if (globalNumPhone != null && globalNumPhone != "") {
        phoneActivo = true;
      }
      return phoneActivo;
    }
    ...
    key: "modSaldo",
    value: function modSaldo(type, title, price, priceOffer) {
      modalnosaldo.tipo(type);
      modalnosaldo.title = title;
      modalnosaldo.price = price;
      modalnosaldo.priceOffer = priceOffer;
      modalnosaldo.openModal();
    }
  }, {
    key: "modRegister",
    value: function modRegister(title, thisHref, idPackage, priceOffer, price, typePackage) {
      modalregistro.startSteps = true;
      modalregistro.title = title;
      modalregistro.thisHref = thisHref;
      modalregistro.idPackage = idPackage;
      modalregistro.priceOffer = priceOffer;
      modalregistro.price = price;
      modalregistro.typePackage = typePackage;
      modalregistro.openModal();
    }
  }, {
    key: "modConfirmation",
    value: function modConfirmation(title, thisHref, price, priceOffer, typePackage, catPaquete, idPackage) {
      modalconfirmation.title = title;
      modalconfirmation.thisHref = thisHref;
      modalconfirmation.price = price;
      modalconfirmation.priceOffer = priceOffer;
      modalconfirmation.typePackage = typePackage;
      modalconfirmation.catPaquete = catPaquete;
      modalconfirmation.idPackage = idPackage;
      modalconfirmation.openModal();
    }
  }, {
    key: "controls",
    value: function controls() {
      ...
      var that = this;
      var buyBt = this.buyBt;

      if (that.BuyActive) {

        jQuery(buyBt).on('click', function (e) {
          e.preventDefault();

          that.activeBuys();
          var thisHref = jQuery(this).attr('href');
          var title = jQuery(this).attr('data-title');
          var idPackage = jQuery(this).attr('data-packageid');
          var price = parseInt(jQuery(this).attr('data-price'));
          var priceOffer = jQuery(this).attr('data-price_offer');
          var typePackage = jQuery(this).attr('data-type');
          var balance = parseInt(globalBalanceEntero);

          //console.log(`Este es el precio ${price}`);

          // Segment
          var catPaquete;
          if (jQuery(this).parents('.wrrp_tabs_shop').length) {
            var catPaq = jQuery(this).parents('.wrrp_tabs_shop').attr('id');
            catPaquete = jQuery('#' + catPaq.replace('tab_', '')).html();
          } else {
            catPaquete = 'Campaña';
          }

          if (GetUserId() == 'anonymousId') {
            typePackage = 'Sin OTP';
          }

          //console.log(`typePackage es ${typePackage}`);

          var buyProduct = {
            category: 'Landing Paquetes',
            label: typePackage + " - " + idPackage + " - " + title + " - " + price,
            value: price,
            currency: 'COL'
          };

          segmentSend.userId = GetUserId();
          segmentSend.event = 'LCP - Comprar - ' + catPaquete;
          segmentSend.returnProperties = buyProduct;
          segmentSend.sendSegment();
          ...
          if (that.phoneActivo()) {
            ...
            //console.log(`aquí ${typePackage}`);

            switch (typePackage) {
              case 'ADELANTA TU PAQUETE':
                that.modConfirmation(title, thisHref, price, priceOffer, typePackage, catPaquete, idPackage);
                break;
              default:
                if (balance < price) {
                  // recargas
                  that.modSaldo('saldo', title, price, priceOffer);
                } else {
                  that.modConfirmation(title, thisHref, price, priceOffer, typePackage, catPaquete, idPackage);
                  // aqui va el llmado a confirmacion
                }
            }
          } else {
            // anonimo otp
            that.modRegister(title, thisHref, idPackage, priceOffer, price, typePackage);
          }
        });
      } // termina boton
    }
    ...
    key: "activeBuys",
    value: function activeBuys() {
      var that = this;
      that.BuyActive = false;
      setTimeout(function () {
        that.BuyActive = true;
      }, 5000);
    }
...
var fnCookieCode = function () {
  function fnCookieCode() {
    _classCallCheck(this, fnCookieCode);

    this.contador = 0, this.intervalCookie;
  }

  _createClass(fnCookieCode, [{
    key: "deleteCookie",
    value: function deleteCookie() {
      var now = new Date();
      document.cookie = 'activecode=; expires= ' + now + '; path=/;';
    }
  }, {
    key: "cookieExist",
    value: function cookieExist() {
      var thereIsCookie = false;
      var theCookies = document.cookie;
      if (theCookies.indexOf('activecode') != -1) {
        thereIsCookie = true;
      } else {
        thereIsCookie = false;
      }
    ...
    value: function createCookie() {
      ...
      var now = new Date();
      var time = now.getTime();
      var expireTime = time + 1000 * (60 * 2);
      now.setTime(expireTime);
      //console.log(now.toGMTString());
      document.cookie = 'activecode=ok; expires=' + now.toGMTString() + ';path=/';
      //document.cookie = 'sesionactiva=ok; path=/';
    }
  }, {
    key: "checkCookieExist",
    value: function checkCookieExist() {
      var that = this;
      var exist = true;
      this.intervalCookie = setInterval(function () {
        if (!that.cookieExist()) {
          //console.log('no existe la cookie');
          clearInterval(that.intervalCookie);
          modalRegistraNumero.alertaModal.removeClass('blue_alert').addClass('red_alert').css('display', 'block').find('p').html('El código que ingresaste ya expiró, te invitamos a solicitar uno nuevo');
          modalRegistraNumero.muevealerta();
        } else {
          //console.log('contador'+that.contador++); 
        }
      }, 1000);
    }
  }]);
...
var fnCookieExpiro = function () {
  function fnCookieExpiro() {
    _classCallCheck(this, fnCookieExpiro);

    this.contador = 0, this.intervalCookie;
  }

  _createClass(fnCookieExpiro, [{
    key: "deleteCookie",
    value: function deleteCookie() {
      var now = new Date();
      document.cookie = 'sesionactiva=; expires= ' + now + '; path=/;';
    }
  }, {
    key: "cookieExist",
    value: function cookieExist() {
      var thereIsCookie = false;
      var theCookies = document.cookie;
      if (theCookies.indexOf('sesionactiva') != -1) {
        thereIsCookie = true;
      } else {
        thereIsCookie = false;
      }
    ...
    value: function createCookie() {
      ...
      now.setTime(expireTime);
      //console.log(now.toGMTString());
      document.cookie = 'sesionactiva=ok; expires=' + now.toGMTString() + ';path=/';
      //document.cookie = 'sesionactiva=ok; path=/';
    }
  }, {
    key: "checkCookieExist",
    value: function checkCookieExist() {
      var contador = 0;
      var exist = true;
      var urlDestroy = returnUrlDrupal("tigopaquete/destroy");
      var that = this;
      this.intervalCookie = setInterval(function () {
        if (!that.cookieExist()) {
          /*
           ...
           */
          clearInterval(that.intervalCookie);
          modalexpiro.openModal();
        } else {
          /*
          console.log('contador '+ contador++);
          console.log('si existe la cookie');
          */
        }
      }, 1000);
    }
  ...
  return fnCookieExpiro;
}();

var esmobile = new siMovil();
var solonumeros = new isNumberKey();
var mainAlert = new fnMainAlert();
var segmentSend = new assembleSegment();

var ingresaPhone1 = new phoneFormat(jQuery('#num_lin'));
var ingresaPhone1Modal = new phoneFormat(jQuery('#numlinsin'));
var iniShowPhone = new phoneFormat();

var modalRegistraNumero = new modalRegisterNumber();
var modalregistro = new modRegNumNoBal();
var modSuscripciones = new modalSuscripciones();
var modalnosaldo = new modalSinSaldo();
var modalexpiro = new modalExpiro();
var modalconfirmation = new modalConfirmation();

var captcha1 = new fnCaptcha('captcha1', 'phone');
captcha1.arrIds = ['reg_numcaptcha1', 'reg_numcaptcha2'];
var captcha2 = new fnCaptcha('captcha2', 'otp');
captcha2.arrIds = ['reg_otpcaptcha1', 'reg_otpcaptcha2'];

// buy button
var buyBts = new fnBuyButton(jQuery('.container_shopweb'));
buyBts.controls();
// buy button

var delSus = new deleteSuscripcion();

// mensaje otp
var sendMessageOtp = new otpMessage();
// mensaje otp
// valida codigo otp
var justOtp = new newOtp();
// valida codigo otp

var newCookieCode = new fnCookieCode();
var newCookie = new fnCookieExpiro();

function currentProtocol() {
  return location.protocol;
}

function returnUrlDrupal(url) {
  var urlDrupal = Drupal.url(url);
  return urlDrupal;
}

function GetUserId() {
  var userId = '';
  globalNumPhone == '' ? userId = 'anonymousId' : userId = globalNumPhone;
  /*
    if(globalNumPhone == ''){
      userId = 'anonymousId'
    } else{
...
(function ($) {
  $.fn.shopWeb = function (options) {
    ...
    var defaults = {
      ...
      tabActive: 'navegar',
      phoneActivo: false,
      servicioMessage: false,
      servicioMessage2: true,
      BuyActive: true,
      // respuesta
      packageId: '',
      packageTitle: '',
    ...
    var wrrPackages = $(this); // contenedor paquetes

    wrrPackages.each(function () {

      // Recyclable functions

      // elipsis
      /*
      $('.ul_paquetes_shop li').each(function(){
       var titLi = $(this).find('.tit_header_paquete_shop h3');
       var	newElip	=	new	createEllipsis($(titLi), 10);
       newElip.controls();
      });
      */
      // elipsis
      // comportamiento inputs
      if ($('.wrrp_campo').length) {
        for (var i = 0; i < $('.wrrp_campo').length; i++) {
          var thisType = $('.wrrp_campo').eq(i).attr("data-type");
          switch (thisType) {
            case 'text':
              var newActiveInput = new activeInput($('.wrrp_campo').eq(i));
              newActiveInput.controlInput();
              break;
            case 'select':
              var newActiveInput = new activeInput($('.wrrp_campo').eq(i));
              newActiveInput.controSelect();
              break;
          }
        }
      }
      ...
      // Funciones TigoShop
      // modalRegisterNumber
      $('#mod_confirmation .close_modal').on('click', function (e) {
        e.preventDefault();
        modalconfirmation.closeModal();
      });
      // modalRegisterNumber
      // modalRegisterNumber
      $('#bt-contver').on('click', function (e) {
        e.preventDefault();
        modalRegistraNumero.submitForm($(this));
      });
      // modalRegisterNumber

      // modRegNumNoBal

      $('#mod_reg_sinbal .close_modal').on('click', function () {
        modalregistro.closeModal($(this));
      });
      $('#bt-regsinbal').on('click', function () {
        modalregistro.stepsModal($(this));
      });

      // modRegNumNoBal

      // active input
      function showPhoneMovil(parent) {
        this.parent = parent, this.btShow = this.parent.find('.bt_showphone'), this.contPhone = this.parent.find('.wrrp_camponumlin'), this.controls = function () {
          var that = this;
          this.btShow.on('click', function () {
            if ($(this).hasClass('active')) {
              $(this).removeClass('active');
              that.contPhone.slideUp();
            } else {
              $(this).addClass('active');
              that.contPhone.slideDown();
            }
          });
        };
      }

      var newShowMovil = new showPhoneMovil($('.header_shopweb'));
      newShowMovil.controls();

      // active input 

      if ($('.container_tabs_shop').length) {
        var findActive = function findActive() {
          var contActive = false,
              tabActive = void 0;
          $('.wrrp_tits_tabs li a').each(function () {
            if ($(this).hasClass('active')) {
              contActive = true;
          ...
          if (contActive) {
            newActiveTabs.thisActive = tabActive;
            newActiveTabs.controls();
          } else {
        ...
        // codigo home shop web
        // desactiva enter para que no refresque la pagina 
        $("#form_numlin").submit(function () {
          return false;
        });
        // desactiva enter para que no refresque la pagina 
        // si ya hizo OTP
        if (options.numPhone != null && options.numPhone != "") {
          options.phoneActivo = true;
        }
        // si ya hizo OTP
        /* Eliminar suscripcion */
        $('.btdelete_sus').on('click', function (e) {
          e.preventDefault();
          var liParent = $(this);
          var idPackage = $(this).attr('data-id');
          var nameSus = $(this).attr('data-name');
          var priceSus = $(this).attr('data-price');
          delSus.fnDelSus(idPackage, liParent);
          // Segment
          var deactivateSubscription = {
            category: 'Landing Paquetes',
            label: nameSus,
            value: parseInt(priceSus),
            currency: 'COL'
          };
          segmentSend.userId = GetUserId();
          segmentSend.event = 'LCP - Desactivar suscripción';
          segmentSend.returnProperties = deactivateSubscription;
          segmentSend.sendSegment();
          // Segment    
        });

        /* Eliminar suscripcion */
        // bt terminos condiciones
        $(document).on('click', '.bt_terminos', function (e) {
          e.preventDefault();
          // Segment
          var viewPolicies = {
            category: 'Landing Paquetes',
          ...
          segmentSend.userId = GetUserId();
          segmentSend.event = 'LCP - Ver términos';
          segmentSend.returnProperties = viewPolicies;
          segmentSend.sendSegment();
          // Segment
        });
        // bt terminos condiciones
        // bt sin saldo home
        $(document).on('click', '.bt-sinsaldo', function (e) {
          e.preventDefault();
          // Segment
          var goToRechargeLanding = {
            category: 'Landing Paquetes',
          ...
          segmentSend.sendSegment();
          // Segment
          var phoneNumber = options.numPhone;
          var linkRecarga = 'http://www.tigo.com.co/servicios/recarga?client_id=042884dcdee8d77469c9d32fd5c53e6a&msisdn=' + phoneNumber;
          window.location.replace(linkRecarga);
        });
        // bt sin saldo home
        // slider promociones
        var controlsdest = $('.slider_dest_shop li').length;

        if (controlsdest > 0) {

          // Segment
          for (var i = 0; i < $('.slider_dest_shop li').length; i++) {
            var btPromo = $('.slider_dest_shop li').eq(i).find('.buybutton');

            var loadCampaign = {
              category: 'Landing Paquetes',
              label: btPromo.attr('data-packageid') + ' - ' + btPromo.attr('data-title') + ' - ' + btPromo.attr('data-price'),
              value: parseInt(btPromo.attr('data-price')),
              currency: 'COL'
            };

            segmentSend.userId = GetUserId();
            segmentSend.event = 'LCP - Visualizar campaña';
            segmentSend.returnProperties = loadCampaign;
            segmentSend.sendSegment();
          }
          // Segment
        }

        if ($('.slider_dest_shop li').length > 1) {
          $('.slider_dest_shop').bxSlider({
            minSlides: 1,
            maxSlides: 1,
            pager: controlsdest,
            infiniteLoop: false,
        ...
        // slider promociones
        // crea tabs  
        var newActiveTabs = new activeTabs($('.wrrp_tits_tabs'), options.numPhone);
        findActive();
        ingresaPhone1.thisKeyUp();
        ingresaPhone1Modal.thisKeyUp();

        if ($('#num_lin').val().length > 0) {
          $('#num_lin').parent('.wrrp_campo').addClass("active");
          if (ingresaPhone1.valPhone) {
            $('#bt_consultar').removeClass('inactive');
          } else {
            $('#bt_consultar').addClass('inactive');
          }
        }

        $('#num_lin').bind('input propertychange', function () {
          if (!esmobile.itsMobile) {
            ingresaPhone1Modal.thisKeyUp();
            // aqui algo
          }
        ...
        //var firstCaptcha = null;

        $('#bt_consultar').on('click', function (e) {
          e.preventDefault();
          var body = $("html, body");
          var topPackages = $('.tit_paquetes').position();
          if (!$(this).hasClass('inactive')) {
            if (options.phoneActivo) {
              //body.stop().animate({scrollTop:topPackages.top});
              window.location.replace(Drupal.url('tigopaquete/destroy'));
            } else {
              if (captcha1.activeCaptcha) {
                if (captcha1.validateCaptcha(grecaptcha.getResponse(captcha1.firstCaptcha))) {
                  sendMessageOtp.optSend($("#num_lin").attr('data-number'), 'msotp1');
                  grecaptcha.reset(captcha1.firstCaptcha);
                  grecaptcha.reset(captcha1.secondCaptcha);
                  captcha1.resetCaptcha();
                } else {
                  $('#num_lin').parent('.wrrp_campo').addClass('errorcampo');
                  $('#num_lin').parent('.wrrp_campo').find('.alerta_errorcampo').html('Dale un click en "No soy un Robot" para continuar con la transacción..');
                }
              } else {
                sendMessageOtp.optSend($("#num_lin").attr('data-number'), 'msotp1');
              }
        ...
        $('#bt_cambiar').on('click', function (e) {
          e.preventDefault();
          var url = Drupal.url('tigopaquete/change');
          window.location.replace(url);
        });

        // consultar home
        // si reconoce linea al inicio
        if (options.phoneActivo) {

          captcha1.destroyCaptcha(); // captcha reinicia en otp
          $('#num_lin').attr('disabled', 'disabled');

          if (newCookie.cookieExist()) {
            newCookie.checkCookieExist();
          } else {
            newCookie.createCookie();
            newCookie.checkCookieExist();
          }

          //$('.container_shopweb').addClass('phone_activo');
          $('.wrrp_camponumlin').css('display', 'none');
          $('#showphone').html(iniShowPhone.formatoInput(options.numPhone, '#showphone'));
          //var iniNumLin = new phoneFormat();
          //$('#showphone').html(iniNumLin.formatoInput(options.numPhone, '#showphone'));
          $('#num_lin').val(ingresaPhone1.formatoInput(options.numPhone, '#num_lin')).parent().addClass('active');
          ingresaPhone1.valPhone = true;
          //$('#showphone').html(phoneFormat.formatoInput(options.numPhone, '#showphone'));
          //$('#num_lin').val(phoneFormat.formatoInput(options.numPhone, '#num_lin')).parent().addClass('active');
          $('.header_shopweb').addClass('active');
          //$('#bt_consultar').removeClass('inactive');
          //$('#bt_consultar').find('span').html('Cambiar');
          $('#showbalance').html('$' + options.balance);
          $('#showdeuda').html('$' + options.deuda);
          $('.wrrp_infolinea').css('display', 'inlil-block');
          $('.wrrp_camponumlin').css('display', 'none');
          // $('.wrrp_camponumlin .icon_ckeck').css('display', 'block');
          //$('.bt_inimovil').css('display', 'none');
        } else {
          if (newCookie.cookieExist()) {
            newCookie.deleteCookie();
          }
        }
        // si reconoce linea al inicio
        // movil home
        if (esmobile.itsMobile) {
          $('.container_shopweb').addClass('wrrp_shopweb_movil');
          $('.bt_inimovil').on('click', function (e) {
            e.preventDefault();
            $(this).slideUp();
            $('.wrrp_camponumlin').slideDown();
          });
        }
        // movil home
      } // codigo home shop web   
      ...
      $('#bt_susc').on('click', function (e) {
        e.preventDefault();
        modSuscripciones.openModal();
      });
      $(document).on('click', '.header_mod_susc', function () {
        modSuscripciones.acordeonSuscripcion($(this));
      });

      /* Suscripciones */

      // alto modal
      function altoModal(parent) {
        this.parent = parent, this.altoBody = function () {
          var bodyHeight = $(window).height();
          return bodyHeight;
        }, this.topBody = function () {
          var bodyTop = $(window).scrollTop();
          return bodyTop;
        }, this.controls = function () {
          var that = this;
          var timeAnim = 0.3;
          //if(siEsMovil() && that.altoBody()<560){
          //timeAnim = 0;
          //}
          TweenLite.to($(that.parent), 0.3, { css: { height: that.altoBody(), top: that.topBody() } });
          ...
          window.addEventListener('scroll', function () {
            TweenLite.to($('.cover_container_modales'), 0.3, { css: { top: 0 } });
            if (timer !== null) {
              clearTimeout(timer);
            }
            timer = setTimeout(function () {
              TweenLite.to($(that.parent), 0.5, { css: { height: that.altoBody(), top: that.topBody() } });
            }, 150);
          }, false);
          $('.content_modal input[type="tel"]').focus(function () {
            $(this).parents('.wrapper_modal').find('.wrapper_alerta').slideUp();
          });
        };
      }
      var newAltoModal = new altoModal($('.container_modales'));
      newAltoModal.controls();

      // alto modal 

      // pagina respuesta

      if ($('.wrrp_respuesta').length) {
        var msgAlert = '';
        var typeAlert = '';
        var ResponseEvent = "";
        $('#phone_resp').html('Tu línea Tigo: ' + iniShowPhone.formatoInput(options.numPhone));
        if (options.alert_responce == 1) {
          // Segment
          ResponseEvent = 'LCP - Compra exitosa';
          // Segment
          msgAlert = '¡Tu compra ha sido exitosa!';
          typeAlert = 'green';
        } else {
          // Segment
          ResponseEvent = 'LCP - Compra no exitosa';
          // Segment
          msgAlert = 'En el momento no podemos procesar tu compra. Te enviaremos un SMS con los detalles de la transacción.';
          typeAlert = 'red';
          $('#bt-buyaganin').find('span').html('VOLVER A PAQUETES');
        }
        // Segment
        var successfulBuyPacket = {
          category: 'Landing Paquetes',
          label: globalPackageType + " - " + options.packageId + " - " + globalPackageTitle + " - $" + globalPackageCost,
          value: globalPackageCost,
          currency: 'COL'
        };
        segmentSend.userId = GetUserId();
        segmentSend.event = ResponseEvent;
        segmentSend.returnProperties = successfulBuyPacket;
        segmentSend.sendSegment();
        // Segment
        mainAlert.positionalert = 'static';
        mainAlert.alerttype = typeAlert;
        mainAlert.msg = msgAlert;
        mainAlert.time = false;
        mainAlert.controls();
        // modulos dudas 
        var modDudas = new modsDudas($('.wrrp_modsopc'));
        modDudas.controls();
        // modulos dudas
      }
      // pagina respuesta
